"""
Simple Dukascopy Data Collector
...
def main():
    print("\n" + "="*60)
    print("  DUKASCOPY DATA COLLECTOR")
    ...
    df_events = df_events[df_events["datetime"] >= cutoff_date]
    print(f"✓ Filtered to {len(df_events)} events from 2020 onwards")
    ...
    for idx, row in tqdm(df_events.iterrows(), total=len(df_events), desc="Processing events"):
        event_data = get_event_data(
            event_id=row["id"],  # ← Matches CSV id column
            event_datetime=row["datetime"],
            currency=row["currency"],
            event_name=row["event"],
            impact=row["impact"],
            actual=row.get("actual", None),  # Include CSV fields
    ...
    print(f"   Avg change: {df_results['change_percent'].mean():.2f}%")
    print(f"   Avg volatility: {df_results['volatility_percent'].mean():.2f}%")
    print(f"\n   Top movers:")
    for _, row in df_results.nlargest(3, "change_percent").iterrows():
        print(f"     {row['pair']}: {row['change_percent']:+.2f}% ({row['event_name']})")
